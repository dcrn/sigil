...
#[derive(Default)]
struct SessionState {
    /// True once cdd_list_contracts or cdd_get_affected_contracts has been called.
    listed: bool,
    /// Contract ids for which cdd_get_contract has been called in this session.
    read_ids: HashSet<String>,
}

pub struct CddServer {
    pub tool_router: ToolRouter<CddServer>,
    pub config: Config,
    session: Mutex<SessionState>,
}

#[tool_handler]
impl ServerHandler for CddServer {
...
impl CddServer {
    pub(super) fn require_listed(&self, tool: &str, contract_id: &str) -> Result<(), String> {
        if !self.session.lock().unwrap().listed {
            Err(error_response(format!(
                "You must call cdd_list_contracts or cdd_get_affected_contracts before calling {tool} for '{contract_id}'."
            )))
        } else {
            Ok(())
        }
    ...
    pub(super) fn require_read(&self, tool: &str, contract_id: &str) -> Result<(), String> {
        if !self.session.lock().unwrap().read_ids.contains(contract_id) {
            Err(error_response(format!(
                "You must call cdd_get_contract for '{contract_id}' before calling {tool}."
            )))
        } else {
            Ok(())
        }
...
impl CddServer {
    ...
    #[tool(description = "Retrieve a single contract by id with full detail. When retrieve_file_contents is true, includes the file contents of all files referenced in the contract. Requires a prior cdd_list_contracts or cdd_get_affected_contracts call in the current session.")]
    async fn cdd_get_contract(
        &self,
    ...
    async fn cdd_get_affected_contracts(
        &self,
    ...
    async fn cdd_validate_contract(
        &self,
    ...
    #[tool(description = "Apply partial updates to an existing contract. Unspecified fields are preserved. List fields are replaced wholesale. Returns a diff of what changed. Requires a prior cdd_get_contract call for this contract_id in the current session.")]
    async fn cdd_update_contract(
        &self,
    ...
    #[tool(description = "Delete a contract. Requires a prior cdd_get_contract call for this contract_id in the current session.")]
    async fn cdd_delete_contract(
        &self,
    ...
    async fn cdd_validate_all_contracts(
        &self,
