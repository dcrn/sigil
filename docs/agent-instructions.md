...
1. Call `cdd_list_contracts` to see the contract landscape. **You must call `cdd_list_contracts` or `cdd_get_affected_contracts` before calling `cdd_get_contract` or `cdd_find_related` -- the server enforces this and will reject calls that use an id you haven't discovered through a listing call.**
